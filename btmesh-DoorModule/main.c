  ...
  switch (evt_id) {
    ...
    case gecko_evt_system_external_signal_id:
    {
      ...
      if (((evt->data.evt_system_external_signal.extsignals) & ADC_EXT_EVT_NORTH) !=0)
      {
    	  ADC_window_out();
    	  debounceFlag = true;
    	  if(door_unlock_index < 4)
    	  {
    		  door_unlock_user_key[door_unlock_index] = 1;
    		  char temp[2] = {0};
    		  sprintf(temp,"%d",1);
    		  strcat(dumpString,temp);
    		  LCD_write(dumpString,LCD_ROW_DATA);
    		  //LCD_write("1", LCD_ROW_DATA);
      ...
      if (((evt->data.evt_system_external_signal.extsignals) & ADC_EXT_EVT_SOUTH) !=0)
      {
    	  ...
    	  if(door_unlock_index < 4)
    	  {
    		  door_unlock_user_key[door_unlock_index] = 2;
    		  char temp[2] = {0};
    		  sprintf(temp,"%d",2);
    		  strcat(dumpString,temp);
    		  LCD_write(dumpString,LCD_ROW_DATA);
//    		  LCD_write("2", LCD_ROW_DATA);
      ...
      if (((evt->data.evt_system_external_signal.extsignals) & ADC_EXT_EVT_WEST) !=0)
      {
    	  ...
    	  if(door_unlock_index < 4)
    	  {
    		  door_unlock_user_key[door_unlock_index] = 3;
    		  char temp[2] = {0};
    		  sprintf(temp,"%d",3);
    		  strcat(dumpString,temp);
    		  LCD_write(dumpString,LCD_ROW_DATA);
//    		  LCD_write("3", LCD_ROW_DATA);
      ...
      if (((evt->data.evt_system_external_signal.extsignals) & ADC_EXT_EVT_EAST) !=0)
      {
    	  ...
    	  if(door_unlock_index < 4)
    	  {
    		  door_unlock_user_key[door_unlock_index] = 4;
    		  char temp[2] = {0};
    		  sprintf(temp,"%d",4);
    		  strcat(dumpString,temp);
    		  LCD_write(dumpString,LCD_ROW_DATA);
//    		  LCD_write("4", LCD_ROW_DATA);
      ...
      if (((evt->data.evt_system_external_signal.extsignals) & ADC_EXT_EVT_CENTER) !=0)
      {
    	  ...
    	  if(door_unlock_index < 5)
    	  {
					  ...
					  door_unlock_user_key[3] == door_unlock_key[3] )
    		  {
    			  ...
    			  memset(door_unlock_user_key,0,4);
    			  memset(dumpString,0,4);
    		  ...
    		  else
    		  {
    			  ...
    			  LCD_write("Pattern Wrong", LCD_ROW_MODULE_STATE);
    			  LCD_write("", LCD_ROW_DATA);
    		  }
    	  }
    	  else
    	  {
    		  ...
    		  LCD_write("", LCD_ROW_DATA);
    		  memset(dumpString,0,4);
    		  LCD_write("Ent Pattern", LCD_ROW_MODULE_STATE);
    	  }
...
//      req = &(server_evt->data.evt_mesh_generic_server_client_request);
      if(door_locked_onApp == 0)
      {
    	  if(door_pattern_matched == 1)
    	  {
				  ...
				  handle_button_press(1);
				  gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(10000), TIMER_ID_DOOR_LOCK, 1);
				  door_pattern_matched = 0;
    	  }
    	  else
    	  {
